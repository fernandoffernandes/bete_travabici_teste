...
#define ENABLE_CS       IO_RC1_SetLow()
#define DISABLE_CS      IO_RC1_SetHigh()
...
void PCD_WriteRegister(unsigned char addr, unsigned char value) 
{
    ...
    readData = SPI1_ExchangeByte((addr << 1)&0x7E); //Equivalente a SPI.transfer((addr<<1)&0x7E);
    readDummy = SPI1_ExchangeByte(value); // Equivalente a  SPI.transfer(val);
...
void PCD_WriteRegisters(unsigned char addr, unsigned char count, unsigned char *values) 
{
    ...
    {
        readDummy = SPI1_ExchangeByte(values[index]);
    }
...
{   
    ...
    unsigned char value;
    readData = SPI1_ExchangeByte(((addr << 1)&0x7E) | 0x80);        // Equivalente a SPI.transfer(((addr<<1)&0x7E) | 0x80);
    value = SPI1_ExchangeByte(0x00);                                // Equivalente a val = SPI.transfer(0x00);
                        ...
                        unsigned char rxAlign)          ///< Only bit positions rxAlign..7 in values[0] are updated.
{
    ...
    if (rxAlign) { // Only update bit positions rxAlign..7 in values[0]
        ...
        unsigned char value = SPI1_ExchangeByte(((addr << 1)&0x7E) | 0x80); // Equivalente a SPI.transfer(((addr<<1)&0x7E) | 0x80);
        // Apply mask to both current value of values[0] and the new data in value.
        values[0] = (values[0] & ~mask) | (value & mask);
        index++;
    }
    while (index < count) {
        values[index] = SPI1_ExchangeByte(((addr << 1)&0x7E) | 0x80); // Equivalente a SPI.transfer(((addr<<1)&0x7E) | 0x80);
        index++;
    }
    values[index] = SPI1_ExchangeByte(0x00); // Equivalente a val = SPI.transfer(0x00);
...
void PCD_Init(void) 
{
    ...
    IO_RC6_SetDigitalInput(); //rst entrada
    if (IO_RC6_GetValue() == false) {
        IO_RC6_SetDigitalOutput(); //rst salida
        IO_RC6_SetLow(); // Make shure we have a clean LOW state.
        __delay_us(20); // 8.8.1 Reset timing requirements says about 100ns. Let us be generous: 2?sl
        IO_RC6_SetHigh(); // Exit power down mode. This triggers a hard reset.
        __delay_ms(50);
        hardReset = true;
    }
    if (!hardReset) 
